...
const int k = 26; // максимальная глубина

// Дерево поиска(Бор, префиксное дерево) будем хранить в таких структурах
struct Vertex {
...
vector<Vertex> bohr; // Массив дерева поиска
vector<string> pattern; // Шаблон

// Создание узла дерева на основе символа
...
// Инициализация бора (создание корня)
void bohr_ini() {
    bohr.push_back(make_bohr_vrtx(0, '$'));
}

// Создание бора на основе строки
...
// Переход по ребру или по суффиксной ссылке
int get_auto_move(int v, char ch) {
...
// Поиск по бору (автомату)
// Процедура хождения по "хорошим" суффиксным ссылкам
// v - текущая позиция автомата
// i - эта позиция оканчивается на i-ю букву в слове
void check(int v, int i) {
    for (int u = v; u != 0; u = get_suff_flink(u)) {
        if (bohr[u].flag) cout << "Совпадение на индексе " << i - pattern[bohr[u].pat_num].length() << " слова " << pattern[bohr[u].pat_num] << endl;
    }
}

// Поиск по бору строки s
